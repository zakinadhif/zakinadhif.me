...
export default function Tag({ tag, postsGroupedByYear }) {
    return (
        <Layout>
            <h1 className="text-xl font-medium mb-4 text-bluemoon">Posts with tag: <span className="underline">{tag}</span></h1>
            <ul>
            {postsGroupedByYear?.map(([year, posts]) => (
                ...
                {posts.map((post) => (
                    <li className="ml-4">
                        <time className="text-smoke mr-6" datetime={post.date}>{post.date}</time>
                        <Link href={`/posts/${post.slugStr}`}><a className="hover:underline underline-offset-1">{post.title}</a></Link>
                    </li>
                ))}
                </>
            ))}
