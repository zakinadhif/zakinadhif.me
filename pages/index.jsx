...
export default function Home({ allPostsData }) {
  return (
    <Layout>
      ...
      <h2 className="text-3xl text-bluemoon mb-4 font-medium">Writing</h2>
      <ul className="space-y-1 font-medium">
        {allPostsData.map(({ slugStr, date, title }) => (
          <li className="" key={slugStr}>
            <span className="text-smoke mr-8">{date}</span>
            <span className="underline underline-offset-1"><Link href={`/${slugStr}`}><a>{title}</a></Link></span>
          </li>
        ))}
      </ul>
      <h2 className="text-3xl text-bluemoon mb-4 font-medium mt-7">Projects</h2>
      <ul className="space-y-1 font-medium">
